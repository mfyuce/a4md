...
#ifndef ARTERY_MDSERVICECPM_H_
#define ARTERY_MDSERVICECPM_H_
...
namespace artery {
    ...
    class MisbehaviorDetectionServiceCpm : public ItsG5Service {
    public:

        ~MisbehaviorDetectionServiceCpm() override;
    ...
    private:
        ...
        std::shared_ptr<vanetza::asn1::Cpm> mLastSentCpm;
        ...
        void visualizeCpmPosition(vanetza::asn1::Cpm cpm, const libsumo::TraCIColor &color,
                                  const std::string &idPrefix);

        void initializeParameters();

        std::vector<std::shared_ptr<vanetza::asn1::Cpm>> getSurroundingCpms(StationID_t senderStationId);

        std::shared_ptr<CheckResult> checkCpm(const shared_ptr<vanetza::asn1::Cpm> &message);

        void detectMisbehavior(const shared_ptr<vanetza::asn1::Cpm> &message);

        vector<shared_ptr<vanetza::asn1::Cpm>> getOverlappingCpms(const DetectedSender &detectedSender);
